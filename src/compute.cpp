  ...
  tlist(nullptr), vbiasall(nullptr)
{
  ...
  //  extra_dof = 0.0;
  if (logfile) fprintf(logfile, "HX test Compute ExtraDof: %f\n", extra_dof);
  dynamic_user = 0;
  fix_dof = 0;
...
void Compute::adjust_dof_fix()
{
  ...
  // setting dof_fix
  if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix nfix: %d\n", nfix);
  ...
  for ( int i = 0 ; i < nfix; i++){
 	s = i;
    if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix loop 1 nfix: %d\n", nfix);
    if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix loop 1 i: %d\n", s);
    if (fix[s]->dof_flag)
      fix_dof += fix[s]->dof(igroup);
	  if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix loop ExtraDof %f\n", extra_dof);
	  if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix loop FixDof %d\n", fix_dof);
    if (logfile) fprintf(logfile, "HX test Compute AdjustDofFix loop 2 nfix: %d\n", nfix);
  }

//  if (logfile) fprintf(logfile, "HX test Compute adjust fix dof %d\n", fix_dof);
}
...
// check reset_extra_dof before and after reset
  if (logfile) fprintf(logfile, "HX test Compute ResetExtraDof 1 ExtraDof: %f\n", extra_dof);
  extra_dof = domain->dimension;
  if (logfile) fprintf(logfile, "HX test Compute ResetExtraDof 2 ExtraDof: %f\n", extra_dof);
}
